...
void Camera::set_persp_view(float field_of_view, int width, int height, float near_clipping_plane,
...
void Camera::set_ortho_view(int width, int height, float near_clipping_plane,
                            float far_clipping_plane) {
    //Calculate the projection matrix
    ortho_proj_m = Mat4::PROJECT_ORTHO(near_clipping_plane, far_clipping_plane, (float) width,
                                       (float) height);
}
...
void Camera::update_view_matrix() {
    ...
    if (use_quaternion)
        ...
        transform =
                Mat4::ROT_TRANS(angles, pos, &right, &up, &forward) * Mat4::ROTATE(flip_y_and_z);
