...
int Skeleton::load_animation(const char *filepath) {
    ...
    const unsigned int **new_anims_data = (const unsigned int **) malloc(
            sizeof(int *) * anim_count);
    ...
    anims_IT[cindex] =
            (float *) all_anims_raw_data[cindex] + 2 + (16 * bone_count * anim_lengths[cindex]);
    return 1;
}
...
Mat4 Entity_Bone_Joint::get_world_transform(bool modify_trans) {
    ...
    transform = parent_skel->get_bone_transform(parent_bone_index) *
                parent_skel->get_bone_rest_transform(parent_bone_index);
