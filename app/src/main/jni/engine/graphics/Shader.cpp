    ...
    for (int i = 0; i < param_count; i++) {
        ...
        switch (param_types[i]) {
            ...
            case PARAM_BONE_INDICES:
                ...
                *((GLint *) (param_location[i])) = glGetAttribLocation(gl_program,
                                                                       param_identifiers[i]);
                break;
                //Uniforms
            case PARAM_CUBE_MAP:
            ...
            case PARAM_TEST_FIELD:
                param_location[i] = (GLint *) malloc(sizeof(GLint));
                *((GLint *) (param_location[i])) = -1;
                *((GLint *) (param_location[i])) = glGetUniformLocation(gl_program,
                                                                        param_identifiers[i]);
                break;
                //Matrix Arrays
...
int Shader::bind_shader_value_by_index(int index, void *data, int extra_data) {
    if (*((GLint *) (param_location[index])) == -1) {
        LOGW("Warning: shader \"%s\" param location at index %d has not been set (type: %d)",
             vert_shader_name, index, param_type[index]);
        return 0;
    }
    GLint loc = 0;
    GLuint uloc = 0;
    switch (param_type[index]) {
        ...
        case PARAM_BONE_MATRICES: {
            ...
            glUniformMatrix4fv((((GLint *) param_location[index])[0]), extra_data, GL_FALSE,
                               ((float *) data));
            break;
        }
        case PARAM_BONE_IT_MATRICES: {
            ...
            //}
            glUniformMatrix3fv((((GLint *) param_location[index])[0]), extra_data, GL_FALSE,
                               ((float *) data));
            break;
        }
        case PARAM_BONE_WEIGHTS:
        ...
        case PARAM_TEST_FIELD:
            loc = *((GLint *) (param_location[index]));
            glUniform4f(loc, ((float *) data)[0], ((float *) data)[1], ((float *) data)[2],
                        ((float *) data)[3]);
            break;
        default:
            break;
    ...
    for (int type = 0; type < GLOBAL_PARAM_COUNT; type++) {
        ...
        switch (type) {
            ...
            case GLOBAL_PARAM_VEC3_DIRLIGHT_COL:
                glUniform3f(loc, global_params[type][0], global_params[type][1],
                            global_params[type][2]);
                break;
                //One float
