...
const char *File_Utils::load_raw_asset(const char *file_path) {
    ...
    char *alloced_data = (char *) malloc(
            sizeof(char) * (file_size + 1));//adding an extra space for null termination
...
void File_Utils::write_savedata(const char *file_name) {
    ...
    FILE *f = fopen(file_path,
                    "a+");//w for write, a+ for append (used to test/ensure lifetime of stored data)
    if (!f) {
