...
void sl_buffer_callback(SLBufferQueueItf snd_queue, void *c) {
    ...
    for (int i = 0; i < e->MAX_SOUND_SOURCES; i++) {
        ...
        for (int j = 0; j < smpls_to_copy; j++) {
            ...
            Stereo_Sample smp = *((Stereo_Sample *) (source->sound->raw_data) +
                                  (j + source->sound_pos));
            smp.l *= left_falloff;
            smp.r *= right_falloff;
...
#endif
    (*(snd_queue))->Enqueue(snd_queue, e->active_audio_buffer,
                            sizeof(Stereo_Sample) * SND_AUDIO_BUFFER_SIZE);
#ifdef DEBUG_AUDIO_CALLBACK
...
int Audio_Engine::init_sl() {
    ...
    result = (*sl_engine_interface)->CreateOutputMix(sl_engine_interface, &sl_output_mix,
                                                     out_mix_iid_count, out_mix_iid,
                                                     out_mix_req);
    if (result != SL_RESULT_SUCCESS) {
    ...
    format_pcm.channelMask =
            SL_SPEAKER_FRONT_RIGHT | SL_SPEAKER_FRONT_LEFT;//SL_SPEAKER_FRONT_CENTER for mono audio
    ...
    result = (*sl_engine_interface)->CreateAudioPlayer(sl_engine_interface, &sl_audio_player,
                                                       &audio_source, &data_sink,
                                                       snd_plyr_iid_count, snd_plyr_iids,
                                                       snd_plyr_reqs);
    if (result != SL_RESULT_SUCCESS) {
    ...
    result = (*sl_audio_player)->GetInterface(sl_audio_player, SL_IID_PLAY,
                                              &sl_audio_player_interface);
    if (result != SL_RESULT_SUCCESS) {
        LOGE("sl_audio_player GetInterface play failed");
        return 0;
    }
    result = (*sl_audio_player)->GetInterface(sl_audio_player, SL_IID_BUFFERQUEUE,
                                              &sl_buffer_queue_interface);
    if (result != SL_RESULT_SUCCESS) {
    ...
    result =
            (*sl_buffer_queue_interface)->RegisterCallback(sl_buffer_queue_interface,
                                                           sl_buffer_callback,
                                                           this);//(void*)&osl_engine
    if (result != SL_RESULT_SUCCESS) {
    ...
    (*sl_buffer_queue_interface)->Enqueue(sl_buffer_queue_interface, active_audio_buffer,
                                          sizeof(short) * SND_AUDIO_BUFFER_SIZE);
    //Swap the audio buffers
...
int Audio_Engine::play_sound_sample(Sound_Sample *sound_sample, Entity *ent, Vec3 position,
                                    int sound_priority, float vol) {
...
int Audio_Engine::play_sound(Sound_Sample *sound, Entity *ent, Vec3 pos, int sound_priority,
                             float volume) {
    if (instance == NULL) {
