...
struct Quat {
    float w;
    //x,y,z being the i,j,k components
    Vec3 v;
...
struct Mat4 {
    ...
    //Alternate ROT_TRANS, assigns forward right and up vectors that are passed in
    static Mat4 ROT_TRANS(const Vec3 &angles, const Vec3 &pos, Vec3 *right, Vec3 *up,
                          Vec3 *forward) {
        Quat pitch(angles.x, Vec3::RIGHT());
        //Rotating angles.y about UP vector for yaw
        Quat yaw(angles.y, Vec3::UP());
    ...
    static Mat4 PROJECT_PERSPECTIVE(const float near, const float far, const float aspect,
                                    const float fov) {
        Mat4 result;
    ...
    static Mat4 PROJECT_ORTHO(const float near, const float far, const float width,
                              const float height) {
